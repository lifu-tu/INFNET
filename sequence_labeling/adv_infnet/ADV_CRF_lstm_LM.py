...
class GAN_CRF_model(object):
	...
	def __init__(self,  We_initial,   params):
		...
		l_LM_in = lasagne.layers.InputLayer((None, None, 26))
                ...
                l_LM_lstm = lasagne.layers.LSTMLayer(l_LM_in, 2*hidden, mask_input=l_LM_mask)
		...
		pos_end_target = y_in[T.arange(length_index.shape[0]), length_index]
                ...
                pos_lm_mask_var = T.concatenate([pos_tmp[:,0, 0].reshape((-1,1)), gmask], axis=1)

                pos_LM_out = lasagne.layers.get_output(l_LM, {l_LM_in:pos_y_lm_in, l_LM_mask:pos_lm_mask_var})
                pos_LM_out = pos_LM_out.reshape((-1, length+1, 26))

                pos_LM_cost = T.sum(T.log(T.sum(pos_LM_out[:,:-1,:]*pos_y_lm_out[:,:-1,:], axis=2)+ eps)*gmask, axis=1)
		

    		pos_cost = target_energies[-1] + T.sum(T.sum(local_energy*y_in, axis=2)*gmask, axis=1) + T.dot( pos_end_target, Wyy[:-1,-1]) + params.lm*pos_LM_cost
		...
		sos = neg_tmp*(start.dimshuffle('x', 0, 1))
                ...
                neg_lm_mask_var = T.concatenate([neg_tmp[:,0, 0].reshape((-1,1)), gmask], axis=1)

                neg_LM_out = lasagne.layers.get_output(l_LM, {l_LM_in:neg_y_lm_in, l_LM_mask:neg_lm_mask_var})
                neg_LM_out = neg_LM_out.reshape((-1, length+1, 26))

                neg_LM_cost = T.sum(T.log(T.sum(neg_LM_out[:,:-1,:]*neg_y_lm_out[:,:-1,:], axis=2)+ eps)*gmask, axis=1)
		...
		neg_end_target = predy[T.arange(length_index.shape[0]), length_index]
                neg_cost = negtarget_energies[-1] + T.sum(T.sum(local_energy*predy, axis=2)*gmask, axis=1) + T.dot(neg_end_target, Wyy[:-1,-1])+ params.lm*neg_LM_cost
		...
		test_cost = -T.mean(neg_cost_test)+ params.l3*T.mean(ce_hinge)-params.lm*T.mean(neg_LM_cost)
	...
	def train(self, trainX, trainY, devX, devY, testX, testY, params):	
		...
		bestdev1 = -1
        	...
        	try:
				...
				#print devacc, negscore, posscore, margin
				testacc = self.test_time(testx0, testx0mask, testy0, testmaxlen)
				end_time2 = time.time()
				if bestdev < devacc:
					bestdev = devacc
					...
					#tmp_d_para = [p.get_value() for p in self.d_params]
                                	#saveParams( tmp_d_para , params.outfile + '_energy_network.pickle')
				...
				#####################################################################################################
                		#final returning for the inference network
                		#####################################################################################################
				...
				for i in xrange(1):
							...
							if (i_index%10==0): 
									...
									test_pred = self.test_time2(testx0, testx0mask, testmaxlen)
									f = open('000LM_Test_QA_' + params.outfile, 'w')
